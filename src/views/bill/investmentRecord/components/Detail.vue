  ...
  >
      ...
      <el-card shadow="never" class="table-wrapper" v-loading="table.loading.value">
        <el-table :data="table.list.value" row-key="id" style="height:50vh">
          ...
          <el-table-column prop="amount" label="金额(U)" min-width="120" />
          <el-table-column prop="refund_amount" label="退款金额(U)" min-width="120" />
          <el-table-column prop="issue" label="期号" min-width="100" />
...
import api, { type Form as DetailForm, Query as DetailQuery } from "@/api/bill/investmentRecord";
...
/** 表格实例 */
const table = new TableInstance<DetailForm>({getList:()=>api.detail(unref(record_id))}, queryParams, 20, queryFormRef);
