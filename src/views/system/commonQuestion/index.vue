<template>
  <el-card class="el_card" v-loading="loading[0]">
    <template #header>
      <h4 style="margin: 0">{{ $t("changJianWenTi") }}</h4>
    </template>
    <content v-model="formData" :keys="['original', 'translation']" @init-before="initBefore" />
    <template #footer>
      <el-button class="button" type="primary" @click="submitHandler" v-hasPerm="['commonQuestion:edit']" :loading="loading[1]">{{ $t("baoCun") }}</el-button>
...
async function getPolicy() {
  ...
  const res = await api.getCommonQuestion();
  formData.value = res;
...
async function submitHandler() {
  ...
  try {
    await api.updatePolicy({ ...unref(formData), type: 5 });
  } finally {
