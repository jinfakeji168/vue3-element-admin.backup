<template>
  <el-card class="el_card" v-loading="loading[0]">
    <template #header>
      <h4 style="margin: 0">{{ $t("guanYuWoMen") }}</h4>
    </template>
    <content v-model="formData" :keys="['original', 'translation']" @init-before="initBefore" />
    <template #footer>
      <el-button class="button" type="primary" @click="submitHandler" v-hasPerm="['aboutUs:edit']" :loading="loading[1]">{{ $t("baoCun") }}</el-button>
...
async function submitHandler() {
  ...
  try {
    await api.updatePolicy({ ...unref(formData), type: 6 });
  } finally {
